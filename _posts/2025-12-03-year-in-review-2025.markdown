...
The foundational pillar of the 2025 roadmap was an infrastructure platform engineered to link consumer wearable biometrics with verified national identity layers. This system bridges Polar AccessLink (biometrics) and SingPass MyInfo (identity), solving a critical challenge of ingesting high-sensitivity health data without exposing identity fields to intermediate states or database administrators.
...
- **Hybrid Encryption & Identity Federation (updated)**: One significant engineering complexity was the handling of NRIC data. A hybrid application-layer encryption flow was implemented: data is encrypted using AES-256 in transit, and the NRIC is persisted in DynamoDB as an RSA-2048–encrypted payload. This ensures that only designated data custodians with private-key access can recover the plaintext NRIC, while backend services only process plaintext in volatile memory during the encryption transformation.
...
- **Stateless Core with Config-Driven Topology**: The control-plane routing layer is stateless at the process level, and all LLM nodes are fully stateless. Endpoints are (re-)discovered and configured via a registry, while conversation state is stored centrally in persistent storage owned by the control plane. For each request, the router reconstructs the conversation from storage and sends the full context to a selected LLM node; these LLM nodes never hold or read conversational state themselves. This keeps the control plane horizontally scalable and simplifies failure recovery: any router instance can handle any request, and failed LLM nodes can be restarted or replaced without migrating in-memory session state.
...
- **Hybrid Encryption & Identity Federation**
  Hybrid Encryption & Identity Federation: Handling NRIC data required extra care. The system encrypts this information before it moves between components, and only an encrypted, locked version is stored in the database. A small group of authorised data custodians hold the private key needed to unlock and read the NRIC; backend services see it in readable form only briefly in memory while it is being encrypted, and never store it in plain text.
