...
- **Hybrid Encryption & Identity Federation (updated)**: One significant engineering complexity was the handling of NRIC data. A hybrid application-layer encryption flow was implemented: data is encrypted using AES-256 in transit between services, and is persisted in DynamoDB as an RSA-2048–encrypted payload. This ensures that only designated data custodians holding on to the private-key can recover the plaintext NRIC, while backend services only process plaintext in volatile memory during the encryption transformation.
