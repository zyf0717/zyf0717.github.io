...
- **Infrastructure as Code & Multi-Environment Strategy**: The entire stack is defined in Terraform, utilizing a modular architecture to enforce strict separation of concerns between networking, storage, and compute. Security (via IAM roles) follows the principle of least privilege. Deployment is via CI/CD, with workspace-based isolation (dev, stg, prd) and a rigid artifact promotion strategy where immutable build artifacts are propagated from staging to production.

- **Hybrid Encryption & Identity Federation**: One significant engineering complexity was the handling of NRIC data. A hybrid application-layer encryption flow was implemented: data is encrypted using AES-256 in transit, and the NRIC is persisted in DynamoDB as an RSA-2048–encrypted payload. This ensures that only designated data custodians with private-key access can recover the plaintext NRIC, while backend services only process plaintext in volatile memory during the encryption transformation.
