...
The theme for 2025 was Systems Engineering—not just writing code, but designing end-to-end systems that are secure by design, resource-efficient, and disciplined in how they handle data. Each project that follows pushes that principle in a different direction.
...
The platform is built as a collection of small services on AWS that only activate when needed, instead of one big always-on server. This makes it easier to scale, isolate problems, and keep the system maintainable over time.
