...
In C, the same is often accomplished using pointers and loops, and while more convoluted, is expected to compile to more performant binary code. My implementation of Hamming distance function in C is as follows:
...
One noteworthy exception is Julia. Despite being dynamically typed (with optional type declarations) and having rather high levels of abstraction (similar to and influenced by Python and R), a just-in-time LLVM compiler compiles Julia code into machine code. As a result, properly written Julia code is extremely performant, [almost comparable to C code](https://julialang.org/benchmarks/). My implementation of Hamming distance function in Julia is as follows:
...
Other languages like Bash and R have different domains, and focus is more on scripting (properly) and vectorization respectively. My implementation of Hamming distance in Bash and R respectively are as follows:
